...
public class ThreadingClass implements Runnable {
	...
	public ThreadingClass(HashMap<String, Double[]> acc, HashMap<String, Double> output, String[] data, int start,
			int end) {
		acc_structure = acc;
		output_hmap = output;
		inputdata = data;
	...
	public void print(HashMap<String, Double> output) {
		...
		for (String key : keys) {
			System.out.println("The Average for key : " + key + " in Nolock execution is : " + output.get(key));
		}
	}
