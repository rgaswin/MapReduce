...
public class Threadclassnosharing implements Runnable {
	...
	private Thread t;
	public HashMap<String, Double[]> acc_structure;
	public HashMap<String, Double> output_hmap;

	// Initialize values specific to this object
	public Threadclassnosharing(String[] data, int start, int end) {
		inputdata = data;
		start_index = start;
		end_index = end;
		acc_structure = new HashMap<String, Double[]>();
		output_hmap = new HashMap<String, Double>();
	}

	// Implement the runnable interface.
	public void run() {
		process_threads(this.inputdata, this.start_index, this.end_index);
		// calc_average(this.acc_structure, this.station_count);
	}
	...
	public void print(HashMap<String, Double> output) {
		...
		for (String key : keys) {
			System.out.println("The Average for key : " + key + " in No sharing execution is : " + output.get(key));
		}
	}
}
